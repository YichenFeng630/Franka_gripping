...
class GraspPipeline:
    ...
    def smooth_trajectory_iptp(self, trajectory, max_vel=0.2, max_accel=0.1):
        ...
        try:
            ...
            total_time = max(8.0, num_points * 0.1)  # At least 8.0s or 0.1s per point (doubled for smoother motion)
        ...
        try:
            ...
            if not self.cartesian_approach_with_fallback():
                ...
                if not self.try_next_candidate_and_retry():
                    ...
                    return
                # If retry succeeded, continue with close/lift
                rospy.loginfo("✓ Candidate retry succeeded, continuing with close/lift...")
    ...
    def cartesian_approach_with_fallback(self):
        """
        Two-stage Cartesian descent: 
        Stage 1: Fast descent to pre-contact position (2cm above object)
        Stage 2: Very slow final descent (only vertical speed allowed)
        
        CRITICAL: Execute once and wait for completion. NO re-planning during descent.
        """
        self.current_state = GraspState.CARTESIAN_APPROACH
        self.publish_status("CARTESIAN_APPROACH")
        rospy.loginfo("\n" + "="*60)
        rospy.loginfo("PHASE 3: Two-Stage Cartesian Descent")
        rospy.loginfo("="*60)
        
        # ============================================================
        # STAGE 1: Fast descent to pre-contact height (2cm above target)
        # ============================================================
        rospy.loginfo("\nSTAGE 1: Fast descent to pre-contact position (10cm above target)")
        rospy.loginfo("-"*60)
        
        # Create pre-contact pose (10cm above target position for safety)
        # Simple approach: just go well above where we want to grasp
        pre_contact_pose = copy.deepcopy(self.current_grasp_pose)
        pre_contact_pose.pose.position.z = self.target_pose.pose.position.z + 0.10  # 10cm above target (safe margin)
        
        rospy.loginfo(f"Target (cube center) z: {self.target_pose.pose.position.z:.4f}m")
        rospy.loginfo(f"Pre-contact z: {pre_contact_pose.pose.position.z:.4f}m (target + 0.10m for safe margin)")
        rospy.loginfo(f"Grasp z: {self.current_grasp_pose.pose.position.z:.4f}m")
        
        # Execute Stage 1 - ONCE only
        fraction1, success1 = self.compute_and_execute_cartesian(
            [pre_contact_pose.pose],
            self.cartesian_step_size,
            "Stage 1 (fast descent to 2cm)"
        )
        
        if not success1 or fraction1 < 0.70:
            rospy.logerr(f"Stage 1 failed (fraction={fraction1:.2f})")
            return False
        
        if fraction1 < 0.90:
            rospy.logwarn(f"Stage 1 completed with fraction {fraction1:.2%}, continuing...")
        
        rospy.loginfo(f"Stage 1 SUCCESS: Reached pre-contact position")
        
        # ============================================================
        # Micro-pause at pre-contact position
        # ============================================================
        rospy.loginfo("Micro-pause at pre-contact (0.5s)...")
        rospy.sleep(0.5)
        
        # ============================================================
        # STAGE 2: Very slow final descent (only vertical velocity)
        # ============================================================
        rospy.loginfo("\nSTAGE 2: Very slow final descent to cube center")
        rospy.loginfo("-"*60)
        
        # Debug: print all relevant positions
        rospy.loginfo(f"Pre-contact pose: x={pre_contact_pose.pose.position.x:.4f}, y={pre_contact_pose.pose.position.y:.4f}, z={pre_contact_pose.pose.position.z:.4f}")
        rospy.loginfo(f"Current grasp pose: x={self.current_grasp_pose.pose.position.x:.4f}, y={self.current_grasp_pose.pose.position.y:.4f}, z={self.current_grasp_pose.pose.position.z:.4f}")
        rospy.loginfo(f"Target pose: x={self.target_pose.pose.position.x:.4f}, y={self.target_pose.pose.position.y:.4f}, z={self.target_pose.pose.position.z:.4f}")
        
        # Final descent: stay vertically aligned (same xy as pre_contact, descent in z only)
        # This ensures a pure vertical descent without horizontal movement
        # CRITICAL: Descend to current_grasp_pose.z (which includes grasp_offset_z), NOT target_pose.z
        final_grasp_pose = copy.deepcopy(pre_contact_pose)
        final_grasp_pose.pose.position.z = self.current_grasp_pose.pose.position.z  # Use grasp height, not cube center
        
        rospy.loginfo(f"Final grasp pose: x={final_grasp_pose.pose.position.x:.4f}, y={final_grasp_pose.pose.position.y:.4f}, z={final_grasp_pose.pose.position.z:.4f}")
        rospy.loginfo(f"Descent distance: {pre_contact_pose.pose.position.z - final_grasp_pose.pose.position.z:.4f}m")
        
        # Use VERY conservative execution parameters
        # Ultra-conservative: 10% velocity, 5% acceleration
        self.move_group.set_max_velocity_scaling_factor(0.10)
        self.move_group.set_max_acceleration_scaling_factor(0.05)
        
        rospy.loginfo("Executing final descent with extreme caution (vel=0.10, accel=0.05)")
        
        fraction2, success2 = self.compute_and_execute_cartesian(
            [final_grasp_pose.pose],  # Vertical descent to target z
            self.cartesian_step_size / 2,    # Even finer step size
            "Stage 2 (ultra-slow final descent)"
        )
        
        if not success2 or fraction2 < 0.60:
            rospy.logerr(f"Stage 2 failed (fraction={fraction2:.2f})")
            rospy.logerr("Possible reasons: target too close to table, joint limits, or collision")
            return False
        
        if fraction2 < 0.80:
            rospy.logwarn(f"Stage 2 completed with low fraction ({fraction2:.2%}), but continuing...")
        
        rospy.loginfo(f"Stage 2 SUCCESS: Reached grasp position at cube center (fraction={fraction2:.2%})")
        rospy.loginfo("="*60)
    ...
    def compute_and_execute_cartesian(self, waypoints, step_size, description):
        ...
        try:
            ...
            self.smooth_trajectory_iptp(
                ...
                max_vel=0.15,     # 15% max velocity (reduced for smoother motion)
                max_accel=0.08    # 8% max acceleration (reduced for smoother motion)
            )
            ...
            self.move_group.set_max_acceleration_scaling_factor(0.10)  # 10% max accel (reduced from 15%)
            self.move_group.set_max_velocity_scaling_factor(0.15)      # 15% max velocity (reduced from 20%)
            
            # Set very relaxed goal tolerance for Stage 2 descent
            if "Stage 2" in description or "descent" in description.lower():
                self.move_group.set_goal_tolerance(0.10)  # 0.1 rad tolerance for final descent
                rospy.loginfo(f"{description}: Using relaxed goal tolerance (0.10 rad)")
            
            rospy.loginfo(f"{description}: Executing with accel=0.10, vel=0.15 (ultra-smooth motion)")
    ...
    def try_next_candidate_and_retry(self):
        ...
        Returns True if candidate was successfully loaded and approach completed.
        NOTE: This only handles up to approach - caller must continue with close/lift/etc.
        """
        ...
        if self.load_candidate(self.current_candidate_idx):
            ...
            if self.plan_to_pre_grasp():
                # Try approach - if successful, return True so caller continues with close/lift
                result = self.cartesian_approach_with_fallback()
                if result:
                    return True  # Success - caller will continue with close_gripper, lift, etc.
                # If approach failed, try next candidate recursively
                rospy.logwarn(f"Candidate {self.current_candidate_idx} approach failed, trying next...")
